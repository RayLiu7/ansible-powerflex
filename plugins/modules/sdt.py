...
""" Ansible module for managing SDT on Dell Technologies (Dell) PowerFlex"""
...
module: sdt
version_added: '2.6.0'
short_description: Manage SDT on Dell PowerFlex
description:
- Managing SDT on PowerFlex storage system includes
...
class PowerFlexSDT(PowerFlexBase):
    ...
    def __init__(self):
        ...
        self.result = dict(changed=False, sdt_details={})
    ...
    def validate_ip_parameter(self, sdt_ip_list):
        ...
        if sdt_ip_list is None or len(sdt_ip_list) == 0:
            error_msg = (
                ...
                "sdt_ip_list as 'ip' and 'role' for Create/Modify "
                "operations."
            )
            LOG.error(error_msg)
    ...
    def validate_create(self, protection_domain_id, sdt_ip_list, sdt_name):
        ...
        if sdt_name is None or len(sdt_name.strip()) == 0:
            error_msg = "Please provide valid sdt_name value for the creation of SDT."
            LOG.error(error_msg)
            self.module.fail_json(msg=error_msg)

        if protection_domain_id is None:
            error_msg = (
                "Protection Domain is a mandatory parameter "
                "for creating an SDT. Please enter a valid value."
            )
            LOG.error(error_msg)
            self.module.fail_json(msg=error_msg)
    ...
    def to_modify(
        ...
        if maintenance_mode is not None:
            if (
                ...
                modify_dict["maintenance_mode"] = "active"
            if (
                maintenance_mode == "inactive"
                and sdt_details["maintenanceState"] != "NoMaintenance"
            ):
                modify_dict["maintenance_mode"] = "inactive"
    ...
    def classify_ip_list_change(self, sdt_ip_list, sdt_details_ip_list):
        # identify IPs to add/remove or roles to update
    ...
    def validate_names(self, sdt_params):
        ...
        for param in params:
            if param is not None and len(param.strip()) == 0:
                error_msg = (
                    "Provide valid value for name for the "
                    "creation/modification of the SDT."
                )
                LOG.error(error_msg)
                self.module.fail_json(msg=error_msg)
...
class SDTChangeIPListHandler:
    def handle(self, sdt_obj, sdt_params, sdt_details, create_sdt_flag, sdt_ip_list):
        if (
            ...
            if ips_to_add:
                ip_added = sdt_obj.add_ip(sdt_details, ips_to_add)
            if roles_to_update:
                role_changed = sdt_obj.update_role(sdt_details, roles_to_update)
            if ips_to_remove:
                ip_removed = sdt_obj.remove_ip(sdt_details, ips_to_remove)

            if ip_added or ip_removed or role_changed:
                sdt_obj.result["changed"] = True
...
class SDTModifyHandler:
    def handle(self, sdt_obj, sdt_params, sdt_details, create_sdt_flag, sdt_ip_list):
        if sdt_params["state"] == "present" and sdt_details and not create_sdt_flag:
            ...
            if modify_dict:
                modify_sdt_flag = sdt_obj.modify_sdt_attributes(
    ...
    def handle(self, sdt_obj, sdt_params, sdt_details, protection_domain_id):
        ...
        if sdt_params["state"] == "present" and not sdt_details:
            if sdt_params["sdt_new_name"] or sdt_params["maintenance_mode"]:
                ...
                LOG.error(error_msg)
                sdt_obj.module.fail_json(msg=error_msg)
